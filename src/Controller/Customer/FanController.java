...
public abstract class FanController {
    ...
    public static boolean canAppreciate() {
        String qry = "SELECT Count(*) from CustomerAdmires where ClientID=\"" + iMuzaMusic.getLoggedUser().getID() + "\"";
        ResultSet rs = iMuzaMusic.getDB().query(qry);
        iMuzaMusic.log("Getting count for customer admires: " + iMuzaMusic.getLoggedUser().getID());
        try {
            if (rs.next()) {
